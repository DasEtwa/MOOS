...
class ReleaseMetadataTests(unittest.TestCase):
    ...
    def test_project_metadata_and_bundle_identifier(self) -> None:
        ...
        self.assertEqual(project_metadata.version, "0.1.0")
        self.assertEqual(project_metadata.build_version, "1")
    ...
    def test_invalid_or_inconsistent_build_number_is_rejected(self) -> None:
        ...
        for replacement in ("0", "01", "abc"):
            with self.subTest(replacement=replacement), tempfile.TemporaryDirectory() as directory:
                ...
                project.write_text(
                    original.replace("CURRENT_PROJECT_VERSION = 1;", f"CURRENT_PROJECT_VERSION = {replacement};"),
                    encoding="utf-8",
                )
                with self.assertRaises(metadata.MetadataError):
