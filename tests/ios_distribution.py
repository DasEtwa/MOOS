...
class ReleaseWorkflowTests(unittest.TestCase):
    def test_normal_ci_cannot_publish(self) -> None:
        ...
        self.assertIn("python3 tests/ios_simulator.py", workflow)
        self.assertIn("scripts/select-ios-simulator.py", workflow)
        self.assertIn("steps.simulator.outputs.udid", workflow)
        self.assertEqual(workflow.count('"scripts/select-ios-simulator.py"'), 2)
        self.assertEqual(workflow.count('"tests/ios_simulator.py"'), 2)
        self.assertEqual(workflow.count("id: simulator"), 1)
        self.assertEqual(workflow.count("steps.simulator.outputs.udid"), 2)
        self.assertIn("build-for-testing", workflow)
        self.assertIn("test-without-building", workflow)
        self.assertNotIn("name=iPhone 16,OS=18.5", workflow)
